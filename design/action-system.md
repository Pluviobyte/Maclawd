...
最初判断「阻塞在设计上」，但把数算清楚之后前提就变了：

| | 取景 | 窗口 | px / 单位 |
| --- | --- | --- | --- |
| 主形态 | 45 单位 | 128px | ≈ 2.84 |
| mini | 16 单位 | 48px | 3.00 |

**mini 不是把角色缩小，而是把镜头推近、裁掉场景只留演员**——角色在屏幕上
的实际大小几乎不变，一个像素都不用重画。这和整套 scene-first 语言同源，
所以「48px 下可读的角色形态」这个设计难题根本不存在。

贴边位把角色右推 7 单位，右半边被 viewBox 右界裁掉，于是
**静止时只露出一只眼睛，探出来才看得到两只**。这是 mini 全部的表意手段：
姿态与露出多少，没有道具。左边缘由外壳整体 `scaleX(-1)` 复用，不另画资产。

动作定为 **8 个**而非 6 个，两处与最初设想不同：

- 加了 `mini.busy`。原方案里 thinking / working / delegating 全都落进 idle 档，
  那样 mini 下根本看不出 Claude 在不在干活——而「一眼看到状态」正是这个产品的主张。
- `needs_owner` 与 `error` **不合并**。「要你决定」需要立刻响应，「出错了」不一定；
  为省一个动作而丢掉这个区分不划算。

契约见 `design/mini-actions.json`（含 39 → 8 的**穷举**收敛表）。
素材由 `scripts/make-mini-svgs.mjs` 从 `characterContract` 派生，
不手写——手写等于把角色几何复制第九份，改主形态时 mini 会静默留在旧几何上。

**踩到的两个坑**（都是渲染出来才看见的，几何测试证明不了）：

1. 爪子任何**向左**位移都会和躯干拉开一条缝。带道具的主形态里那读作
   「伸手去够」，mini 没有道具可够，离体的爪子只会读作渲染错误。
2. 躯干**长期倾斜**在 `crispEdges` 下渲染成锯齿平行四边形。主形态里 body
   旋转只出现过一次（`recovering-body`，瞬时 ±5°）。「垮掉」改用低电量那套
   「下沉 + 纵向压缩」的词汇表达。

两条都已写成测试强制执行，不靠记忆。

**仍未做**：22px 菜单栏标记还是程序化占位（`MenuBarController.markImage`），
与 mini 无关，另行处理。
