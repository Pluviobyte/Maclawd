...
| 触发 | 动作 | 模式 | 恢复规则 |
| --- | --- | --- | --- |
| 单击 | Poke Squish | 2.2s 短反应 | 回到触发前主状态 |
| 双击/连点 | Surprised Hop | 2.4s 短反应 | 回到触发前主状态 |
| 开始拖拽 | Hanging Loop | 按住循环 | 松手进入 `Drop Wobble` |
...
获取。`Curtain Peek` 是产品行为，不应由模型事件误触发。V5.3 保留纵向帘幕，
拖拽改为中央吊环；点击、双击、落地和低电量全部移除软垫，直接用身体形变表达。
